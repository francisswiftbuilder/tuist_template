...
private func scanExtensionGroups() -> [(String, [String])] {
	...
	for name in children.sorted() {
		...
		let subModules = subChildren.compactMap { subName -> String? in
			...
			guard fm.fileExists(atPath: subPath, isDirectory: &sIsDir), sIsDir.boolValue else {
				return nil
			}
			let sourcesPath = subPath + "/Sources"
			var srcIsDir: ObjCBool = false
			guard fm.fileExists(atPath: sourcesPath, isDirectory: &srcIsDir), srcIsDir.boolValue else {
				return nil
			}
			return subName
		}.sorted()
...
private func emitLayerEnum(_ lr: LayerResult) -> String {
	if lr.layer == .appExtension {
		...
		var directCases: [LayerResult.ModuleCase] = []
		var nestedGroups: [(name: String, cases: [LayerResult.ModuleCase])] = []
		for g in lr.groups {
			if g.cases.count == 1, g.cases[0].ident == swiftIdentifier(g.name) {
				directCases.append(g.cases[0])
			} else {
				nestedGroups.append((name: swiftIdentifier(g.name), cases: g.cases))
			}
		}
		var lines: [String] = []
		lines.append("// MARK: \(lr.layer.markTitle)")
		lines.append("public extension Module {")
		if directCases.isEmpty {
			lines.append("\tpublic enum \(lr.layer.nestedEnumName) {")
		} else {
			lines.append(
				"\tpublic enum \(lr.layer.nestedEnumName): String, CaseIterable, \(proto) {"
			)
		}
		for mc in directCases {
			if mc.ident == mc.original {
				lines.append("\t\tcase \(mc.ident)")
			} else {
				lines.append("\t\tcase \(mc.ident) = \"\(mc.original)\"")
			}
		}
		for g in nestedGroups {
			lines.append("\t\tpublic enum \(g.name): String, CaseIterable, \(proto) {")
			for mc in g.cases {
				if mc.ident == mc.original {
					lines.append("\t\t\tcase \(mc.ident)")
				} else {
	...
	let proto = protocolName(for: lr.layer)
	lines.append("public extension Module {")
	lines.append("\tpublic enum \(enumName): String, CaseIterable, \(proto) {")
	for mc in lr.cases {
		if mc.ident == mc.original {
			lines.append("\t\tcase \(mc.ident)")
		} else {
	...
	for lr in results {
		if lr.layer == .appExtension {
			...
			for g in lr.groups {
				...
				if g.cases.count == 1, g.cases[0].ident == grpIdent {
					let mc = g.cases[0]
					directLines.append(
						"\tpublic static var \(lowercasedFirst(mc.ident)): Module.AppExtension { .\(mc.ident) }"
					)
					continue
				}
				var groupLines: [String] = []
				groupLines.append(
					"extension ModuleAppExtensionType where Self == Module.AppExtension.\(grpIdent) {"
				)
				for mc in g.cases {
					groupLines.append(
						"\tpublic static var \(lowercasedFirst(mc.ident)): Module.AppExtension.\(grpIdent) { .\(mc.ident) }"
					)
				}
				groupLines.append("}")
				nestedParts.append(groupLines.joined(separator: "\n"))
			}
			if !directLines.isEmpty {
				var lines: [String] = []
				lines.append("extension ModuleAppExtensionType where Self == Module.AppExtension {")
				lines.append(contentsOf: directLines)
				lines.append("}")
				parts.append(lines.joined(separator: "\n"))
			}
			parts.append(contentsOf: nestedParts)
			continue
		}
		...
		for mc in lr.cases {
			lines.append(
				"\tpublic static var \(lowercasedFirst(mc.ident)): Module.\(enumName) { Module.\(enumName).\(mc.ident) }"
			)
		}
		lines.append("}")
		parts.append(lines.joined(separator: "\n"))
...
private func run() {
	...
	var isDir: ObjCBool = false
	let hasAppSources =
		fm.fileExists(atPath: "Projects/App/Sources", isDirectory: &isDir) && isDir.boolValue
	...
	do {
		...
		if hasAppSources {
			...
			fputs("  - App: skipped (Projects/App/Sources not found)\n", stderr)
		}
	} catch {
